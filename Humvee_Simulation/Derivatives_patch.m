function StateDot = Derivatives_patch(State,centroidsxy,xp,yp,zp)
global mark1D
parameters
...
%%%Contact Model of Humvee ADD FRICTION!!!!!
Kuvw = skew(pqr);
XYZ = [0;0;0];
LMN = [0;0;0];
for idx = 1:4
    ...
    zmountainTire1 = interppatch(centroidsxy,xp,yp,zp,rTire(1),rTire(2));
...
XYZ = XYZ + [0;kpslide*v;0];
%%%Turning controller to keep the vehicle straight
psic = 0*pi/180;
LMN = LMN + [0;0;kpturn*(psi-psic)+kdturn*r];
...
for idx = 1:2
    ...
    zmountainTire = interppatch(centroidsxy,xp,yp,zp,rTire(1),rTire(2));
...
%%%Turning Controller to mimic friction
LMN_trailer = LMN_trailer + [0;0;kpturn*(psi_trailer-psic)+kdturn*r_trailer];

%%%Add Thrust and Gravity
Fgrav = Kxyz'*[0;0;m*g];
%%$%Assume Thrust overcomes Fgrav
Fgrav(1) = 0;
